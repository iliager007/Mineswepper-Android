                        ...
                        if (started) {
                            if (btn.isMine()) {
                                ...
                                if (myButton.openedBtns.size() == (height * width - countMines)) {
                                    ...
                                    record.id = recordDao.count();
                                    record.time = System.currentTimeMillis() - start;
                                ...
                                for (int y = 0; y < width; ++y) {
                                    for (int t1 = -1; t1 <= 1; ++t1) {
                                        for (int t2 = -1; t2 <= 1; ++t2) {
                                            if ((x + t1) >= 0 && (x + t1) < height && (y + t2) >= 0 && (y + t2) < width && btns.get(x + t1).get(y + t2).isMine())
                                                btns.get(x).get(y).countMines++;
                                        }
                                    }
                                }
                            }
                            for (int t1 = -1; t1 <= 1; ++t1) {
                                for (int t2 = -1; t2 <= 1; ++t2) {
                                    if ((btn.i + t1) >= 0 && (btn.i + t1) < height && (btn.j + t2) >= 0 && (btn.j + t2) < width && !btns.get(btn.i + t1).get(btn.j + t2).isOpened())
                                        btns.get(btn.i + t1).get(btn.j + t2).open();
                                }
                            }
                            started = true;
